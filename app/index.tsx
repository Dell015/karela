...
const BackgroundGradients = () => (
  <View style={StyleSheet.absoluteFillObject}>
    <LinearGradient colors={['rgba(32, 159, 119, 0.08)', 'rgba(15, 15, 15, 1)', 'rgba(124, 242, 5, 0.05)']} style={StyleSheet.absoluteFillObject} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} />
  </View>
);
...
const CustomSplashScreen = ({ onFinish }: { onFinish: () => void }) => {
  ...
  return (
    <Animated.View style={[styles.splashContainer, { opacity }]}>
      <View style={styles.splashLogoContainer}><Image source={LOGO_IMAGE} style={styles.splashLogo} /></View>
      <View style={styles.loadingBarTrack}><Animated.View style={[styles.loadingBarFill, { width: widthInterp }]} /></View>
    </Animated.View>
  );
};
...
export default function Index() {
  ...
  return (
    <View style={styles.container}>
          ...
          renderItem={({ item }) => (
            <View style={[styles.itemContainer, { width }]}>
              ...
              <View style={styles.textContainer}>
                <View style={styles.titleWrapper}><GradientText text={item.title} style={styles.title} /></View>
      ...
      {/* ORIGINAL PAGINATOR POSITION (Above Footer) */}
        ...
        <View style={styles.footerRow}>
          ...
          <View style={styles.centerWrapper}>
            <Animated.View style={[styles.morphedButtonContainer, { 
                ...
                transform: [{ translateX: morphAnim.interpolate({ inputRange: [0, 1], outputRange: [width / 2 - 90, 0] }) }] 
            }]}>
              <TouchableOpacity style={styles.primaryButtonFill} onPress={scrollToNext} activeOpacity={0.8}>
                <View style={styles.buttonTextWrapper}>
                  <Animated.Text style={[styles.primaryButtonText, { opacity: morphAnim.interpolate({ inputRange: [0, 0.3], outputRange: [1, 0] }), position: 'absolute' }]}>Next</Animated.Text>
                  <Animated.Text style={[styles.primaryButtonText, { opacity: morphAnim.interpolate({ inputRange: [0.7, 1], outputRange: [0, 1] }) }]}>Allow Access</Animated.Text>
                </View>
              </TouchableOpacity>
            </Animated.View>
          </View>
