...
const ProgressCircle = ({ progress, size, strokeWidth = 5, color = "#7CF205" }: any) => {
  const animatedProgress = useSharedValue(0);
  const radius = (size - strokeWidth) / 2;
  const circumference = 2 * Math.PI * radius;
  useEffect(() => { animatedProgress.value = withTiming(progress, { duration: 1500 }); }, [progress]);
...
export default function QuestsScreen() {
  ...
  const [activeTab, setActiveTab] = useState<'Standard' | 'Limited'>('Standard');
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedQuest, setSelectedQuest] = useState<any>(null);

  const transition = useSharedValue(0); 
  useEffect(() => { transition.value = withTiming(activeTab === 'Standard' ? 0 : 1, { duration: 300 }); }, [activeTab]);
  ...
  const openDetails = (title: string, xp: string) => {
    setSelectedQuest({ title, xp, rec: "Focus on high-intensity intervals. Keep your back straight and breathe through your nose, Sander." });
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.tabContainer}>
          <Animated.View style={[styles.animatedPill, { width: (width - 62) / 2 }, animatedPillStyle]}>
            <LinearGradient colors={['#32D74B', '#248A3D']} style={StyleSheet.absoluteFill} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} />
          </Animated.View>
          <TouchableOpacity style={styles.tabButton} onPress={() => setActiveTab('Standard')} activeOpacity={1}>
            <Text style={[styles.tabText, { color: activeTab === 'Standard' ? 'white' : '#7CF205' }]}>Standard</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.tabButton} onPress={() => setActiveTab('Limited')} activeOpacity={1}>
            <Text style={[styles.tabText, { color: activeTab === 'Limited' ? 'white' : '#7CF205' }]}>Limited</Text>
          </TouchableOpacity>
        </View>

        <Text style={styles.sectionTitle}>Daily Missions</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
           <QuestCard title="Morning 5km Run" xp="150xp" progress={0.7} onDetails={() => openDetails("Morning 5km Run", "150xp")} />
           <QuestCard title="Sprint Intervals" xp="200xp" progress={0.3} onDetails={() => openDetails("Sprint Intervals", "200xp")} />
        </ScrollView>
        ...
        <View style={styles.achievementsRow}>
           <View style={styles.achievementCard}><MaterialCommunityIcons name="trophy" size={40} color="#3A3A3C" /><Text style={styles.achievementLabel}>First 5K</Text></View>
           <View style={styles.achievementCard}><MaterialCommunityIcons name="fire" size={40} color="#3A3A3C" /><Text style={styles.achievementLabel}>7 Day Streak</Text></View>
        </View>
      </ScrollView>
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setModalVisible(false)}>
          <Animated.View entering={FadeIn.duration(300)} style={styles.modalContent}>
            ...
            <View style={styles.detailsGrid}>
               <View style={styles.detailBox}><MaterialCommunityIcons name="lightning-bolt" size={24} color="#7CF205" /><Text style={styles.detailValue}>{selectedQuest?.xp}</Text><Text style={styles.detailLabel}>Reward</Text></View>
            ...
            <View style={styles.recContainer}>
               <View style={styles.coachHeader}><MaterialCommunityIcons name="account-tie-voice" size={20} color="#7CF205" /><Text style={styles.recTitle}>Karela's Advice</Text></View>
...
const QuestCard = ({ title, xp, progress, onDetails }: any) => (
  <View style={styles.questCard}>
    <View style={styles.cardTop}>
      <View style={{ flex: 1 }}><Text style={styles.cardSub}>Daily Running</Text><Text style={styles.cardTitle}>{title}</Text><Text style={styles.xpTextSmall}>{xp}</Text></View>
      <ProgressCircle progress={progress} size={50} />
    </View>
    <View style={styles.cardButtons}>
      <TouchableOpacity style={{ flex: 2 }}><LinearGradient colors={['#32D74B', '#248A3D']} style={styles.trackBtn}><Text style={styles.btnText}>Track</Text></LinearGradient></TouchableOpacity>
      <TouchableOpacity style={styles.detailsBtn} onPress={onDetails}><Text style={styles.btnText}>Details</Text></TouchableOpacity>
    </View>
  </View>
);
...
const styles = StyleSheet.create({
  ...
  tabContainer: { flexDirection: 'row', backgroundColor: '#1C1C1E', borderRadius: 32, padding: 6, marginBottom: 35, position: 'relative', height: 64 },
  animatedPill: { position: 'absolute', top: 6, left: 6, bottom: 6, borderRadius: 26, overflow: 'hidden' },
  tabButton: { flex: 1, alignItems: 'center', justifyContent: 'center', zIndex: 1 },
  tabText: { fontSize: 18, fontWeight: 'bold' },
  sectionTitle: { color: 'white', fontSize: 22, fontWeight: '600', marginBottom: 18 },
  horizontalScroll: { marginBottom: 35 },
  questCard: { backgroundColor: '#1C1C1E', borderRadius: 28, padding: 22, width: 280, marginRight: 15 },
  cardTop: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 },
  cardSub: { color: '#8E8E93', fontSize: 11 },
  cardTitle: { color: 'white', fontSize: 22, fontWeight: 'bold', marginVertical: 4 },
  ...
  detailsBtn: { backgroundColor: '#3A3A3C', paddingVertical: 12, paddingHorizontal: 18, borderRadius: 22 },
  btnText: { color: 'white', fontSize: 12, fontWeight: 'bold' },
  summaryContainer: { backgroundColor: '#1C1C1E', borderRadius: 28, padding: 24, marginBottom: 35 },
  ...
  circleBase: { width: 32, height: 32, borderRadius: 16, borderWidth: 2, borderColor: '#2C2C2E' },
  circleActive: { borderColor: '#7CF205' },
  dayLabel: { color: '#8E8E93', fontSize: 10, marginTop: 8 },
  achievementsRow: { flexDirection: 'row', gap: 15 },
  achievementCard: { flex: 1, height: 160, backgroundColor: '#1C1C1E', borderRadius: 28, justifyContent: 'center', alignItems: 'center' },
